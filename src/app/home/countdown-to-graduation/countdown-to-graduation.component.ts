...
export class CountdownToGraduationComponent implements OnInit {
    ...
    private sketch(p: any) {
        ...
        let divWidth = window.document.getElementById("counter").clientWidth;
        ...
        // const TEXT_X_OFFSET: number = 10;
        const ALICEBLUE_HEX: string = "f7f3ee";
        p.setup = (): void => {
            p.createCanvas(divWidth, canvasHeight);
        };
        ...
        setInterval((): void => {
            ...
            if (isOrPastGrad) {
                ...
                let heightDiff: number = canvasHeight / (NUMBER_OF_TIME_BARS * 2 + 1);
                const BAR_CONTAINER_MARGIN: number = heightDiff * .2;
                p.strokeWeight(0);
                ...
                barDataArr.forEach((barObj, index, _) => {
                    ...
                    let height: number = index * 2 * heightDiff + heightDiff
